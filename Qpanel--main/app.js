...
// حماية من حقن HTML — تهريب الأحرف الخاصة قبل إدراجها في innerHTML
...
function renderSheetTable(){
  ...
  sheetTypes.forEach(s=>{
    const tr=document.createElement('tr');
    tr.innerHTML=`
      <td><input value="${escapeHtml(s.name)}" data-id="${s.id}" data-f="name" style="min-width:54px"></td>
...
function sheetStats(sheet){
  if(!settings) return {used:0,area:0,util:0,waste:100,meters:0,cost:0,count:0,cuts:0};
...
function buildSheetCanvas(sheet, idx, colorMap, interactive, count){
  ...
  block.innerHTML=`
    <div class="sheet-head">
      <h3>${count>1?`ألواح متطابقة <span class="sheet-mult">×${count}</span>`:`اللوح ${idx+1}`} <span class="dim-note">${escapeHtml(settings.sheetName)} · ${settings.L}×${settings.W} سم</span></h3>
...
function positionPieces(canvas, scale){
  ...
  sheet.pieces.forEach((p,pi)=>{
    ...
    el.innerHTML=`<span class="dim-h">${fmtNum(p.l)}</span>
                  ...
                  ${small?'':`<span class="pname">${escapeHtml(p.name)}${p.rot?' ⟳':''}</span>`}`;
...
async function doExportPDF(opts){
  ...
  if(opts.summary){
    ...
    let costRows = '';
    if(opts.costs){
      costRows = sRow('سعر اللوح الواحد', '$'+(settings.sheetPrice||0).toFixed(2))+
        sRow('تكلفة الألواح ('+t.sheets+' لوح)', '$'+sheetsCost.toFixed(2))+
    ...
    summary.innerHTML=
      '<div class="pdf-page-header">'+
        ...
        '<div><div style="font-size:24px;font-weight:800;color:#8a5e26">IQ Panel</div>'+
      ...
      '<table class="pdf-summary-table">'+
        sRow('الوح الخام', escapeHtml(settings.sheetName)+' — '+settings.L+' × '+settings.W+' سم')+
  ...
  // صفحة لكل لوح: شبكة ثابتة — صورة المخطط + بيانات اللوح + ترتيب القص
  groupSheets().forEach(function(g){
    var sh=g.sheet, i=g.idx, _cnt=g.count;
    var st=sheetStats(sh);
    var page=document.createElement('div'); page.className='pdf-page';
    var _title=_cnt>1?
      'ألواح متطابقة × '+_cnt+' (أرقام '+g.idxs.map(function(x){return x+1;}).join('، ')+') — '+escapeHtml(settings.sheetName)+' '+settings.L+'×'+settings.W+' سم':
      'اللوح رقم '+(i+1)+' — '+escapeHtml(settings.sheetName)+' '+settings.L+'×'+settings.W+' سم';
    ...
    var blockData=buildSheetCanvas(sh,i,colorMap,false,_cnt);
    ...
    // ترتيب عمليات القص — أعمدة متعددة إذا كان القائمة طويلة
    var orderBlock='';
    if(opts.cutOrder){
      var grp={};
      sh.pieces.forEach(function(p){
        var k=sizeKey(p);
        if(!grp[k]) grp[k]={l:(p.origL!=null?p.origL:p.l),w:(p.origW!=null?p.origW:p.w),name:p.name,n:0};
        grp[k].n++;
      });
      var orderItems=Object.values(grp).map(function(g){
        return '<li>قص مقاس <b>'+g.l+'×'+g.w+'</b> سم'+(g.n>1?' <span style="color:#b5803c;font-weight:800">*'+g.n+'</span> (مكرّر '+g.n+' مرات)':'')+(g.name?' — '+escapeHtml(g.name):'')+'</li>';
      }).join('');
      orderBlock='<div class="pdf-cut-order"><b style="color:#8a5e26">ترتيب عمليات القص:</b><ol>'+orderItems+'</ol></div>';
    }

    // تجميع كل البيانات في العمود الأيمن
    dataCol.innerHTML+=orderBlock;

    // شبكة: المخطط + البيانات جنباً إلى جنب
  ...
  var base=(opts.name||settings.planName||'IQ-Panel-مخطط-القص').toString().trim().replace(/[\\/:*?"<>|]+/g,'-')||'IQ-Panel';
  var fname=base+'.pdf';
  try{
